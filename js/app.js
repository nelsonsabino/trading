...
import { supabase, listenToAlarms } from './services.js'; // Importa listenToAlarms do services.js
...
let currentAlarms = []; // Variável para armazenar os alarmes
...
async function initializeApp() {
    ...
    listenToTrades(async (trades) => {
        ...
        displayTrades(activeTrades, marketData, currentAlarms); // Passa os alarmes também
    });

    // Escuta alarmes do Supabase
    listenToAlarms((alarms, error) => {
        ...
        // Quando os alarmes são atualizados, re-renderiza os trades para atualizar a sinalização
        listenToTrades(async (trades) => { // Re-chama listenToTrades para forçar update
            const activeTrades = trades.filter(t => ['POTENTIAL', 'ARMED', 'LIVE'].includes(t.data.status));
            const marketData = await fetchMarketDataForDashboard(activeTrades);
