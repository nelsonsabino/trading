...
// --- FUNÇÃO PARA BUSCAR E MOSTRAR O PREÇO ATUAL ---
async function fetchAndDisplayPrice(coinSymbol) {
    const priceDisplay = document.getElementById('asset-current-price');
    if (!priceDisplay) return;
    
    priceDisplay.textContent = 'A obter preço...';
    try {
        const response = await fetch(`https://api.binance.com/api/v3/ticker/price?symbol=${coinSymbol.toUpperCase()}USDT`);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NOVA FUNÇÃO PARA LIDAR COM OS PARÂMETROS DO URL ---
    async function handleUrlParams() {
        ...
        if (assetSymbolFromUrl && assetInput) {
            try {
                const { data: results, error } = await supabase.functions.invoke('search-coins', { body: { query: assetSymbolFromUrl } });
                if (error) throw error;
                if (results && results.length > 0) {
                    const bestMatch = results[0];
                    selectedCoin = bestMatch;
                    assetInput.value = `${bestMatch.name} (${bestMatch.symbol.toUpperCase()})`;
                    fetchAndDisplayPrice(bestMatch.symbol);
                } else {
                    assetInput.value = assetSymbolFromUrl;
                }
            } catch (err) {
                console.error("Erro ao pré-preencher moeda do URL:", err);
                assetInput.value = assetSymbolFromUrl;
            }
        }
    }
    ...
    handleUrlParams(); // Executa a nova função ao carregar a página
    ...
    setupAutocomplete(assetInput, resultsDiv, (coin) => {
        selectedCoin = coin;
        if (coin) { fetchAndDisplayPrice(coin.symbol); } 
        else { document.getElementById('asset-current-price').textContent = ''; }
    });
